...
namespace DAgents.Synch
{
    public static class SynchUtility
    {
        ...
        /// <param name="db"></param>
        public static void SynchStats(int pid, DateTime dt, ILogger Logger, SynchDBDataContext db, bool preDelete)
        {
            Logger.Log("date " + dt.ToString());
            Logger.Log("campaign pid " + pid);
            #region Delete stats and line items for current pid and date
            {
                if (preDelete)
                {
                    using (SynchDBDataContext db2 = new SynchDBDataContext(true))
                    {
                        var stats = from c in db2.StatEntities
                                    where c.pid == pid && c.stat_date == dt
                                    select c;
                        ...
                        var items = from c in db2.ItemEntities where c.pid == pid && c.name.Contains(dt.ToString("yyyy-MM-dd")) select c;
            ...
            var stats2 = new StatList(XmlGetter.ByAffiliateForASingleDay(pid, dt));
            // List of stats to be turned into line items
            var statItems = new List<StatEntity>();
            #region Process stats
            //var thelist = stats.StatsDetailList.Where(c =>
            //    c.affid != 1
            //    && c.num_sales != 0
            //    && c.num_sub_sales != 0);
            foreach (var statDetail in stats2.StatsDetailList)
            {
                ...
                if (statDetail.leads == 0
                    && statDetail.num_sales == 0
                    && statDetail.num_sub_sales == 0)
                {
                    try
                    {
                ...
                //cur.contextual_impressions = 0;
                //cur.click_thru = 0;
                //cur.post_impression_leads = 0;
                //cur.signups = 0;
        ...
        public static void SynchCampaignListFromDirectTrackToDatabase(ILogger logger)
        {
            ...
            foreach (var campaignItem in campaignsFromDT.CampaignItems)
            {
                ...
                var campaign = CampaignEntity.GetOrCreate(campaignItem.CampaignId, campaignItem);
                //campaign.campaign_name = campaignItem.CampaignName;
                //campaign.campaign_type = campaignItem.CampaignType;
        ...
        public static void SynchStatsForPid(
            ILogger Logger,
            int pid,
            int year,
            int month,
            ...
            bool preDelete)
        {
            ...
            Logger.Log("done parallel processing " + done + " days at " + DateTime.Now + " with " + errors.Count + " errors");
            if (errors.Count > 0)
            {
        ...
        private static void SetPayouts(StatEntity cur, SynchDBDataContext db)
        {
            ...
            if (ap1 != null)
            {
                ...
                else
                {
                    ...
                    {
                        throw new Exception("no affilaite payout");
                    }
                }
            }
