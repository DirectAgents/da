...
namespace EomApp1.Services
{
    public class SyncherService
    {
        ...
        void resourceGetter_GotResource(ResourceGetter sender, string location, XDocument resource)
        {
            using (var model = new DirectAgentsEntities())
            {
                ...
                if (dtResource == null)
                {
                    dtResource = new DirectTrackResource(resource);
