...
namespace EomApp1.Formss.AB2.Model
{
    ...
    {
        public DirectTrackResource() { }

        public DirectTrackResource(XDocument xDoc)
            : this(new XDocumentAdapter(xDoc))
        {
        }

        public DirectTrackResource(IAdaptable<XDocument> adaptable)
        {
            XmlDoc = adaptable.From.ToString();
            Updated = DateTime.Now;
            ResourceName = adaptable.From.Root.Name.LocalName;
            (adaptable as IMapper).MapTo(this);
        }
    }
